...
export function useAIEnhancement() {
  ...
  const enhance = useCallback(async (request: EnhancementRequest): Promise<EnhancementResult | null> => {
    if (!user) {
...
export function usePromptHistory() {
  ...
  const saveToHistory = useCallback((historyItem: {
  ...
  const updateStats = useCallback((result: EnhancementResult, enhancementTime: number) => {
    setStats(prev => {
      ...
      const newStats = {
        ...
        averageEnhancementTime: (prev.averageEnhancementTime * prev.promptsEnhanced + enhancementTime) / newPromptsEnhanced,
